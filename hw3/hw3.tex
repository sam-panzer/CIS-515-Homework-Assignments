...
%If we let $C_i = \prod_{k=1,k \neq i}^{m+1} (\alpha_i - \alpha_k)$,
%we can write 
%\[L_i = \frac{1}{C_i(x-\alpha_i)}\prod_{k=1}^{m+1} (x - \alpha_k) \]

%Then, if we have some linear combination equal to zero,
%\[ 0 = \sum_{i=1}^{m+1} \frac{\lambda_i}{C_i} L_i = 
%\left(\prod_{k=1}^{m+1}(x-\alpha_k) \right) \sum_{i=1}^{m+1}\frac{\lambda_i}{x-\alpha_i}\]
%Since the $\Pi$ product is nonzero and $(x-\alpha_i)$ is nonzero (let's assume
%that $x \neq \alpha_i$ for any $i$), we would need $\lambda_i = 0$ for all $i$.
